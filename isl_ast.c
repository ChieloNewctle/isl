...
__isl_give isl_ast_expr *isl_ast_expr_dup(__isl_keep isl_ast_expr *expr) {
  ...
  case isl_ast_expr_op:
    dup = alloc_op(expr->u.op.op, isl_ast_expr_list_copy(expr->u.op.args));
    break;
...
isl_ast_expr_op_get_args(__isl_keep isl_ast_expr *expr) {
  if (isl_ast_expr_check_op(expr) < 0)
    return NULL;
 ...
 */
static __isl_give isl_ast_expr_list *
isl_ast_expr_op_take_args(__isl_keep isl_ast_expr *expr) {
  isl_ast_expr_list *args;
...
isl_ast_expr_op_restore_args(__isl_take isl_ast_expr *expr,
                             __isl_take isl_ast_expr_list *args) {
 ...
 */
static isl_bool
isl_ast_expr_list_is_equal(__isl_keep isl_ast_expr_list *list1,
...
isl_bool isl_ast_expr_is_equal(__isl_keep isl_ast_expr *expr1,
  ...
  case isl_ast_expr_op:
    if (expr1->u.op.op != expr2->u.op.op)
      return isl_bool_false;
    return isl_ast_expr_list_is_equal(expr1->u.op.args, expr2->u.op.args);
  case isl_ast_expr_error:
    return isl_bool_error;
  }
...
static __isl_give isl_ast_expr *substitute_ids(__isl_take isl_ast_expr *expr,
  ...
  return isl_ast_expr_substitute_ids(expr, isl_id_to_ast_expr_copy(id2expr));
}
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_user_from_expr(__isl_take isl_ast_expr *expr) {
  isl_ctx *ctx;
  isl_ast_node *node;
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_block_from_children(__isl_take isl_ast_node_list *list) {
  isl_ast_node *node;
  isl_ctx *ctx;
...
static isl_stat isl_ast_node_check_block(__isl_keep isl_ast_node *node) {
  return isl_ast_node_check_type(node, isl_ast_node_block, "not a block node");
}
...
isl_ast_node_for_take_body(__isl_keep isl_ast_node *node) {
  isl_ast_node *body;
 ...
 */
static __isl_give isl_ast_node *
isl_ast_node_for_restore_body(__isl_take isl_ast_node *node,
 ...
 */
static __isl_give isl_ast_node *
isl_ast_node_if_take_then_node(__isl_keep isl_ast_node *node) {
 ...
 */
static __isl_give isl_ast_node *
isl_ast_node_if_restore_then_node(__isl_take isl_ast_node *node,
...
#include "isl_ast_node_set_field_templ.c"
     ...
     */
    static __isl_give isl_ast_node *
    isl_ast_node_if_take_else_node(__isl_keep isl_ast_node *node) {
 ...
 */
static __isl_give isl_ast_node *
isl_ast_node_if_restore_else_node(__isl_take isl_ast_node *node,
                                  __isl_take isl_ast_node *child) {
...
#include "isl_ast_node_set_field_templ.c"
    ...
    isl_ast_node_block_take_children(__isl_keep isl_ast_node *node) {
 ...
 */
static __isl_give isl_ast_node *
isl_ast_node_block_restore_children(__isl_take isl_ast_node *node,
...
#include "isl_ast_node_set_field_templ.c"
     ...
     */
    static __isl_give isl_ast_node *
    isl_ast_node_mark_take_node(__isl_keep isl_ast_node *node) {
  isl_ast_node *child;
 ...
 */
static __isl_give isl_ast_node *
isl_ast_node_mark_restore_node(__isl_take isl_ast_node *node,
                               __isl_take isl_ast_node *child) {
...
static __isl_give isl_ast_node *
traverse(__isl_take isl_ast_node *node,
 ...
 */
static __isl_give isl_ast_node_list *
traverse_list(__isl_take isl_ast_node_list *list,
              __isl_give isl_ast_node *(*enter)(__isl_take isl_ast_node *node,
                                                int *more, void *user),
              __isl_give isl_ast_node *(*leave)(__isl_take isl_ast_node *node,
                                                void *user),
...
static char *node_first_str[] = {
    [isl_ast_node_for] = "iterator", [isl_ast_node_mark] = "mark",
    [isl_ast_node_user] = "user",    [isl_ast_node_if] = "guard",
    [isl_ast_node_block] = "",
};
...
static __isl_give isl_ast_node *read_for(__isl_keep isl_stream *s) {
  ...
  free(key);
  if (!is_value && !is_init)
    isl_die(isl_stream_get_ctx(s), isl_error_invalid, "unexpected key",
            return isl_ast_node_free(node));
  if (isl_stream_yaml_next(s) < 0)
    return isl_ast_node_free(node);
...
__isl_give isl_ast_node *isl_stream_read_ast_node(__isl_keep isl_stream *s) {
  ...
  case isl_ast_node_block:
    isl_die(isl_stream_get_ctx(s), isl_error_internal,
            "block cannot be detected as mapping", return NULL);
  case isl_ast_node_for:
    node = read_for(s);
    break;
  case isl_ast_node_mark:
...
static int ast_expr_required_macros(__isl_keep isl_ast_expr *expr, int macros) {
  ...
  isl_ast_expr_list_foreach(expr->u.op.args, &entry_required_macros, &macros);
